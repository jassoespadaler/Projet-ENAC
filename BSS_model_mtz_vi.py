...
#from data_35_new import *
#from Converter_txt_2_py import *
from vi_subsets import compute_S
'''
...
def getModel(f, data):
    ...
    for i in range(1,data['N']+1):
        for j in range(1,data['N']+1):
            if i != j:
                ...
                coefsConstr.append([1.0, -1.0, data['N']])
                sensesConstr.append('L')
    ...
    for i in range(1,data['N']+1):
        for j in range(1,data['N']+1):
            ...
            co = []
            co.append(1.0)
            for h in compute_S(i,j,data['N'], data['Q'], data['q']):
                ind.append(nameToIndex['x[' + str(j) + ']' + '[' + str(h) + ']'])
                co.append(1.0)
            indicesConstr.append(ind)
            coefsConstr.append(co)
            sensesConstr.append('L')
    ...
    for i in range(1,data['N']+1):
        for j in range(1,data['N']+1):
            ...
            co = []
            for h in compute_S(i,j,data['N'], data['Q'], data['q']):
                ind.append(nameToIndex['x[' + str(h) + ']' + '[' + str(i) + ']'])
                co.append(1.0)
            ind.append(nameToIndex['x[' + str(i) + ']' + '[' + str(j) + ']'])
            co.append(1.0)
            indicesConstr.append(ind)
